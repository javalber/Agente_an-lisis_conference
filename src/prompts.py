...
SUMMARY_PROMPTS: dict[str, str] = {
    "excel": (
        ...
        "Notas sobre el formato de los datos:\n"
        "- Las columnas de cada tabla representan períodos (trimestres). NO asumas su "
        "orden: el período más reciente puede estar en la PRIMERA o en la ÚLTIMA "
        "columna según el documento.\n"
        "- Puede que solo algunas hojas (p. ej. 'continuing operations' / pro forma) "
        "lleguen al período más reciente; otras pueden terminar antes.\n"
        "{period_hint}"
        "Céntrate en:\n"
        "- Cifras clave del período MÁS RECIENTE (totales, métricas principales).\n"
        "- Variaciones relevantes vs. el trimestre anterior (% cambio).\n"
